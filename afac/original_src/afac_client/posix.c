...
int DARSHAN_DECL(open)(const char *path, int flags, ...)
{
    ...
    /*second: new a connection, TCP or UDP*/
    c = cli_socket(server_name, server_port, tcp_transport);		// JAY: different io thread share the same port?
...
ssize_t DARSHAN_DECL(read)(int fd, void *buf, size_t count)
{
    ...
    i = cli_try_read_command(c);		// blocked to wait for the requested data
    if(i != 0) 
    {
        fprintf(stderr, "cli_try_read_command failed\n");
        return 0;
