...
int8_t **ost_known_fd_array;
static void ost_known_fd_init()
{
    assert(G_MAX_FDS > 0 && G_SERVER_NUM);
    ost_known_fd_array = calloc(sizeof(void*), G_SERVER_NUM );
	int i;
	for(i=0; i<G_SERVER_NUM; i++)
	{
		ost_known_fd_array[i] = calloc(sizeof(int8_t), G_MAX_FDS );
	}
		
}

static void set_ost_known_fd(int ost_id, int fd)
{
    assert(ost_id >=0 && ost_id < G_SERVER_NUM);
    assert(fd >= 0 && fd < G_MAX_FDS);
    ost_known_fd_array[ost_id][fd] = 1;
}

static void clear_ost_known_fd(int ost_id, int fd)
{
    assert(ost_id >=0 && ost_id < G_SERVER_NUM);
    assert(fd >= 0 && fd < G_MAX_FDS);
    ost_known_fd_array[ost_id][fd] = 0;
}

static int8_t check_ost_known_fd(int ost_id, int fd)
{
    assert(ost_id >=0 && ost_id < G_SERVER_NUM);
    assert(fd >= 0 && fd < G_MAX_FDS);
    return ost_known_fd_array[ost_id][fd];
}
...
static void send_read_request_to_ost(int cfd, int real_fd, scatter_block_info* scatter_blk, int start_offset, int end_offset)
{
    ...
    if(check_ost_known_fd(dest_ost, real_fd))
    {
    ...
    {
        fd_map* my_fd_map = get_fd_map(cfd);// add a ost_open_arg param, and resend cmd
        ost_open_arg *open_arg = &(my_fd_map->open_arg);
        ...
        set_ost_known_fd(dest_ost, real_fd);
...
static void send_write_request_to_ost(int cfd, int real_fd, scatter_block_info* scatter_blk, int offset_in_buf, int start_offset, int end_offset)
{
    int dest_ost = scatter_blk->ost_id == -1 ? MY_SERVER_ID : scatter_blk->ost_id;
...
static int determine_file_primary_ion(char *absolute_path, int my_server_id)
{
    ...
    if(config_param.multiple_ion == 1)
    {
...
ssize_t hack_read(int fd, void *buf, size_t count)
{
    ...
    int num_first_batch_reqs = num_req_osts * 64;
    ...
    //for(cur_blk=0; cur_blk<num_block_metadata; cur_blk++)
    {
        ...
        send_read_request_to_ost(cfd, fd, &scatter_blocks[cur_blk], start_offset, end_offset);
        num_send_block_reqs++;
	...
	fprintf(stderr, "PULL CONTENT:\n%s\n", buf);
                        ...
                        if(num_send_block_reqs < num_block_metadata)
                        {
                            // need send these left write reqs
                            int next_send_blk = last_send_scatter_block_id + 1;
							assert(next_send_blk >= 0 && next_send_blk < num_block_metadata);
                            ...
                            int end_offset = scatter_blocks[next_send_blk].file_block_id == end_block_index ?
                                             (curr_offset + count - 1) % G_BLOCK_SIZE_IN_BYTES : G_BLOCK_SIZE_IN_BYTES - 1;
                            send_read_request_to_ost(cfd, fd, &scatter_blocks[next_send_blk], start_offset, end_offset);
                            num_send_block_reqs++;
							last_send_scatter_block_id = next_send_blk;
                        }
...
ssize_t hack_write(int fd, const void *buf, size_t count)
{
    ...
    for(cur_blk=0; cur_blk<num_block_metadata; cur_blk++)
    {
        ...
        send_write_request_to_ost(cfd, fd, &scatter_blocks[cur_blk], offset_in_buf, start_offset, end_offset);
                ...
                while(handled_bytes < tmp_rbuf_len)
                {
                    ...
                    else
                    {
                        ...
                        if(num_send_block_reqs < num_blocks_access_remotely)
                        {
                            ...
                            send_write_request_to_ost(cfd, fd, &scatter_blocks[next_send_blk], offset_in_buf, start_offset, end_offset);
                            num_send_block_reqs++;
                        }
                        // need modify event?
                        else
