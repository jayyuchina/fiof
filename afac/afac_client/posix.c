...
static void establish_all_connection()
{
    ...
    {
		// if not ION, then no need to establish conn
		if(ost_id % config_param.ion_cn_ratio != 0)	continue;
...
{
	// I need to get the real fd to satisfy some apps.
	// Some apps would check the fd, otherwise they will exist with an error.
	...
	// TODO: add a pseudo fd generator
	// this generator must have an unique start pos,
	// because some true real fd would conflict with the pseudo fd
}
...
{
    int dest_ost = scatter_blk->ost_id;
...
static int determine_ion_id(char *absolute_path, int my_server_id)
{
	...
	if(config_param.ion_with_hash)
	{
		...
		int num_ion = (int)ceil((double)G_SERVER_NUM / (double)config_param.ion_cn_ratio);
    	hv = hv % num_ion;
...
int hack_open(char *path, int flags, ...)
{
	...
	int server_id;	// this is actually the ION ID
	...
	*/
    ...
    if(DEBUG >= 2) fprintf(stderr, "<<< send open cmd to ION[%d]\n", server_id);
    ...
    // map real fd to cfd
    // temporaily do not change this, because some apps may report errors.
    // it is better to change this when testing open performance
...
ssize_t hack_read(int fd, void *buf, size_t count)
{
	...
	*/
    ...
    int num_blocks_access_remotely = num_block_metadata;
